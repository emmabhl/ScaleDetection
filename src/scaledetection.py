...
class ScaleDetectionPipeline:
    ...
    def process_yolo_detections(
        ...
        boxes = (
            detection_results.boxes.xyxy.cpu().numpy()
        ...
        bar_box, label_box, bar_score, label_score = self.match_text_to_bars(
        ...
        scale = self.get_scale(
            scalebar_detection, label_detection, bar_score, label_score
        )
    ...
    def process_atypical_detections(
        ...
        if detection_results["scale_type"] == "graduation_middleunit":
            ...
            # ----------- MATCHING & RESULTS PREPARATION -----------
            scale = self.get_scale(
                scalebar_detection, label_detection, cls_score, cls_score
            )
            ...
            raise ValueError(
                f"Unknown atypical scale bar type: {detection_results['scale_type']}"
            )
        ...
        if len(label_boxes) == 0 or len(bar_boxes) == 0:
            ...
            for label_idx in sorted_label_indices:
                ...
                for bar_idx in sorted_bar_indices:
                    ...
                    bar_center = (
                        (bar_box[0] + bar_box[2]) / 2,
                        (bar_box[1] + bar_box[3]) / 2,
                    )
                    bar_diagonal = calculate_bar_diagonal(bar_box)
                    max_distance = bar_diagonal * self.max_distance_ratio
                    distance = calculate_distance(label_center, bar_center)
...
def main():
    ...
    group.add_argument(
        "--image_dir", type=str, help="Path to folder containing images to process"
    )

    parser.add_argument(
        "--model",
        type=str,
        default="models/yolov8m_train/weights/best.pt",
    ...
    if args.image:
        ...
        if len(image_paths) == 0:
            log.warning(
                f"No images found in directory {args.image_dir} with extensions: {exts}."
            )
            return
    ...
    for img_path in tqdm(image_paths, desc="Processing images"):
        ...
        if not matches:
            ...
            try:
                results = pipeline.process_yolo_detections(
                    image, yolo_results[0], plot_base
                )
            except Exception as e:
        ...
        pipeline.save_results(
            results, os.path.join(args.output_dir, f"{image_name}.json")
        )
